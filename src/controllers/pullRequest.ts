...
const PR_TEMPLATE_SLUG = 'kitchen-sink'
...
export const pullRequest = async (res: FastifyReply) => {
    const template_file_path = join(process.cwd(), 'templates', PR_TEMPLATE_SLUG, 'index.html')
    const template_test_data = join(process.cwd(), 'templates', PR_TEMPLATE_SLUG, 'meta.json')
    ...
    res.type('text/html; charset=utf-8').send(
        (HTML as string).concat(
            `<div id="dynamicItemsDiv"><pre style="background:#101827;color:#FFFFFF;border-radius:6px;padding:4px 4px 4px 4px;"><code>${JSON.stringify(
                dynamicItems,
                null,
                4
            )}</code></pre></div>`
